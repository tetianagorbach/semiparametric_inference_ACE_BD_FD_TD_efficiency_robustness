...
load("../results/results_sim_fixed_sample_size2021_04_20_11_49_37.Rdata")
load("../results/results_sim_fixed_sample_size2021-03-31.Rdata")
sim.results <- as.data.frame(sim.results)
```

```{r, transfrom sim.results, echo=F}
# First, I transform sim.results into a long format and calculate scaled empirical variance and est. bias:
sim.results.long <-  sim.results%>%
        select(dgp, alpha1, beta1, gamma1, gamma2, n, ate,
               ...
               TD = est.td.semipar,   `FD TD BD` = est.eif.all.semipar)%>%
        gather(Method, ate.est, FD:`FD TD BD`, factor_key=TRUE)%>%
        group_by(dgp, Method)%>%
...
plot_wrap <- function(dat){
        ...
        geom_point(aes(group = Method, col= Method)) + 
...
```{r, plot pos, fig.height=10, fig.width=10, echo=F}
        sim.results.long%>% 
        filter(Method !="naive", alpha1 == -1)%>%
        filter(beta1>0, gamma2>0, gamma1>0)%>%
        plot_wrap
        sim.results.long%>% 
        filter(Method !="naive", alpha1 == -0.5)%>%
        filter(beta1>0, gamma2>0, gamma1>0)%>%
        plot_wrap
        sim.results.long%>% 
        filter(Method !="naive", alpha1 ==0 )%>%
        filter(beta1>0, gamma2>0, gamma1>0)%>%
        plot_wrap
        sim.results.long%>% 
        filter(Method !="naive", alpha1 ==0.5 )%>%
...
sim.results%>%
        ...
        select(dgp,  beta1, gamma1, gamma2, n, 
               ...
               `TD BD` = est.eif.td.bd.semipar, `TD FD` = est.eif.fd.td.semipar, `TD BD FD` = est.eif.all.semipar)%>%
