...
GenerateData <- function(n, pc, par.a.c, par.z.a, par.y.zc, sigma.z, sigma.y) {
  ...
  #' @return  A data frame with generated values of the observed variables C, A, Z, Y
        ...
        y <- rnorm(n, cbind(rep(1, n), z, c1 = c, c2 = new_cov) %*% par.y.zc, sigma.y) # Generate the outcome Y (given a, m, c)
        sim.data <- data.frame(cbind(y, a, z, c1 = c, c2 = new_cov[,1]))
...
EstimateIFFrontTwoDoor <- function(cov.vals.all, exposure, intermediate, outcome, est.pc, fit.a.c, fit.z.a, fit.y.zc, astar) {
  ...
  sum.a <- model.matrix.y[, c("X.Intercept.", "z")] %*% y.par.hat[c("(Intercept)", "z")] +
    y.par.hat["c1"] * est.pc + y.par.hat["c2"] * mean(model.matrix.y[, "c2"]) + 
          #### HERE!!!! need to change
  sum.z <- as.matrix(cbind(1, z.mean_astar, as.numeric(colMeans(cov.vals.all)))) %*% y.par.hat
  sum.az <- as.matrix(cbind(ones, z.mean_ind, t(colMeans(cov.vals.all)))) %*% y.par.hat
